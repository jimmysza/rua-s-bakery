...
import { X, ChefHat, Package, Info, ChevronLeft, ChevronRight } from 'lucide-react';
...
const ProductDetailsModal: React.FC<ProductDetailsModalProps> = ({ product, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center md:p-4 overflow-hidden">
      ...
      <div className="relative bg-white w-full h-full md:h-auto md:max-w-2xl md:max-h-[90vh] md:rounded-3xl shadow-2xl overflow-hidden transform transition-all animate-fade-in flex flex-col">
        {/* Gallery Header */}
        <div className="relative h-64 sm:h-72 md:h-80 w-full bg-gray-100 flex-shrink-0">
          <img 
          ...
          <div className="flex px-6 md:px-8 py-3 bg-white space-x-3 overflow-x-auto scrollbar-hide border-b border-gray-50 flex-shrink-0">
            {images.map((img, idx) => (
              <button
                ...
                className={`h-14 w-14 md:h-16 md:w-16 flex-shrink-0 rounded-xl overflow-hidden border-2 transition-all ${
                  idx === activeImgIndex ? 'border-primary scale-105 shadow-md' : 'border-transparent opacity-60'
                }`}
              >
                <img 
                  src={img} 
        ...
        {/* Content Area */}
        <div className="p-6 md:p-8 overflow-y-auto space-y-8 flex-1 pb-10 md:pb-8 scrollbar-hide">
          ...
          <div className="flex items-center justify-between p-5 bg-gray-50 rounded-2xl border border-gray-100">
             ...
             <span className={`font-bold text-sm md:text-base ${product.stock > 0 ? 'text-green-600' : 'text-red-500'}`}>
